...
function setProp(key, value) {
  return function(state) {
    return {
      ...state,
      [key]: isFunction2(value) ? value(state[key]) : value
    };
  };
}
function setProps(props) {
  return function(state) {
    return {
      ...state,
      ...isFunction2(props) ? props(state) : props
    };
  };
}
...
var asap2 = () => debounceTime(0, asapScheduler);
function filterNil() {
...
function propsFactory(key, {
  ...
  return {
    ...
    [`set${normalizedKey}`](value) {
      return function(state) {
        const newVal = isFunction2(value) ? value(state) : value;
        if (newVal === state[key]) {
          return state;
        }
        return {
...
function arrayRemove(arr, items) {
  const toArray2 = coerceArray(items);
  return arr.filter((current) => !toArray2.includes(current));
}
...
//# sourceMappingURL=@ngneat_elf.js.map
