...
class QuizList extends HTMLElement {
  ...
  connectedCallback() {
    ...
    document.addEventListener('quiz-completed', async (_e: any) => {
      console.log("🎉 Quiz completed, refreshing list...");
      
      // Poczekaj chwilę żeby backend zdążył zapisać dane
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userStats = data.data.filter((stat: any) => {
          const hasUser = stat.user && stat.user.id;
          console.log("🔍 Stat user:", stat.user, "Current:", currentUserId);
          return hasUser ? String(stat.user.id) === String(currentUserId) : false;
        });
        
        console.log("🔍 User's statistics:", userStats);
        
        const solvedQuizSetIds = userStats
          .filter((stat: any) => stat.quizSetId && stat.quizSetId !== null)
          .map((stat: any) => String(stat.quizSetId));
          
        console.log("📊 Solved quiz-sets from backend:", solvedQuizSetIds);
        
        // Aktualizuj solvedIds
        this.solvedIds = [...new Set(solvedQuizSetIds)] as string[];
        this.saveSolvedIds();
        console.log("✅ Updated solved IDs:", this.solvedIds);
      }
    } catch (err) {
  ...
  async fetchQuizzes() {
    ...
    try {
      ...
      console.log("Using localStorage and demo data as fallback");
      const localQuizSets = this.getLocalQuizSets();
      this.quizzes = [...localQuizSets, ...this.getDemoQuizSets()];
      this.render();
    }
